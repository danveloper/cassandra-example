...
public class DatapointsRepositoryTaskTest extends AbstractRepositoryTest {
    ...
    public void maxHistory1() {
        ...
        DatapointRepositoryProcessor task = new DatapointRepositoryProcessor(repository);
    ...
    public void testSecond() {
        DatapointRepository repository = new DatapointRepository(cluster, keyspace, "datapoints", 1000);
        final DatapointRepositoryProcessor task = new DatapointRepositoryProcessor(repository);
    ...
    public void test10Second() {
        ...
        task.process(new ProcessCommand(measurement1.timestampMs,Arrays.asList(measurement1)));

        Measurement measurement2 = new Measurement(measurement1);
        measurement2.timestampMs=20000;
        measurement2.value=20;
        task.process(new ProcessCommand(measurement2.timestampMs,Arrays.asList(measurement2)));

        Measurement measurement3 = new Measurement(measurement1);
        measurement3.timestampMs=30000;
        measurement3.value=30;
        task.process(new ProcessCommand(measurement3.timestampMs,Arrays.asList(measurement3)));

        Measurement measurement4 = new Measurement(measurement1);
        measurement4.timestampMs=40000;
        measurement4.value=40;
        task.process(new ProcessCommand(measurement4.timestampMs,Arrays.asList(measurement4)));
